...
# transform the graph into an averaged graph for each depth bin with confidence intervals
# instead of the general look

# I will need to use "m\red depth bin" data set and not just the table

###############################################################


# plot avr individuals ~ depth (20 m bins)

# med - points: 
ggplot(med_depth_bins,aes(x =depth,y= rowSums(med_depth_bins[,first_species:length(med_depth_bins)]))+
         geom_point(size = 2.5, col = "blue")
       geom_line(aes(group=Engagement,color=Engagement))+
         geom_errorbar(aes(ymin=lower,ymax=upper,color=Engagement,width=0.2))
       
       ggplot(med_depth_bins,aes(x=depth,y=abundance))+
         geom_point(size = 2.5, col = "blue") + ggtitle("med sea abundance ~ depth (binned)") 
       
       # red - points: 
       ggplot(red_depth_bins,aes(x=depth,y=abundance))+
         geom_point(size = 2.5, col = "red") + ggtitle("red sea abundance ~ depth (binned)") 
       
       # -----------

       ##################################################################
                    #Its not right to look at this like that!
        # as total abundance and richness for each depth bin
         # it's true to look at avrg or conf intervals
       ################################################################
                        
       
       # total abundance and biomass - not the right way to look at it
       ...
       # continue Tal's code, look for useful codes 
       
       # run the spatial sample based rarefaction
