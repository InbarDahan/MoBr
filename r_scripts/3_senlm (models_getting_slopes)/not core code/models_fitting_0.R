...
# for these script the adapted function msenlm function is required - r script "msenlm_safe function"
...
# check for zero inflation and NA in my data:
     zero_inflation_NA_summary <- combined_data %>%
...
saveRDS(best_models_parameters_a, file = "best_models_parameters_a.rds")
saveRDS(one_model_a, file = "one_model_a.rds")
saveRDS(best_models_aic_a, file = "best_models_aic_a.rds")
...
saveRDS(best_models_parameters_r, file = "best_models_parameters_r.rds")
saveRDS(one_model_r, file = "one_model_r.rds")
saveRDS(best_models_aic_r , file = "best_models_aic_r.rds")
...
# save the models output:
setwd(wd_models)
saveRDS(subsets_taxons_clean, file = "subsets_taxons_clean.rds")
...
# save the models output:
write.csv(best_models_parameters_e, file = "best_models_parameters_e.csv")
saveRDS(best_models_parameters_e, file = "best_models_parameters_e.rds")
saveRDS(one_model_e, file = "one_model_e.rds")
saveRDS(best_models_aic_e , file = "best_models_aic_e.rds")

################################################################
 
# ---> The next code do not work for an unknown reason
# ---> I need to figure it out to extract the best models
# ---> I rewrote the above functions to jump over non working models

###############################################################


# # 4 - summary top models by ordered AIC (with parameters)
# best_models_parameters_e <- purrr::map(taxas, ~ {     # apply on all the taxas in the list
#   multi <- summary(fitted_models_even[[1]])  # pull the fitted models
#   multi[order(multi$AICc),c(4,5,6,24:28)]
# })
# 
# 
...
models_even_fish <- msenlm_safe(models = abundance_models, data = subsets_taxons_clean[[1]], xvar = "depth", yvar = "sigma", conf.level=0.95)
